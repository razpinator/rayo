...
func (lx *Lexer) Next() Token {
	for lx.offset < len(lx.src) {
		...
		case ' ', '\t', '\r':
			...
			return Token{Kind: TokenWhitespace, Value: lx.src[start:lx.offset], Offset: start, Line: lx.line, Col: startCol}
		case '\n':
			lx.offset++
			lx.line++
			lx.col = 1
		...
		case '#':
			...
			return Token{Kind: TokenComment, Value: lx.src[start:lx.offset], Offset: start, Line: lx.line, Col: startCol}
		case '{':
			lx.offset++
			lx.col++
		...
		case '"', '\'':
			...
			return Token{Kind: TokenString, Value: lx.src[start:lx.offset], Offset: start, Line: startLine, Col: startCol}
		default:
			...
			if unicode.IsLetter(rune(ch)) || ch == '_' {
				...
				if _, ok := pythonKeywords[val]; ok {
					return Token{Kind: TokenKeyword, Value: val, Offset: start, Line: lx.line, Col: startCol}
				}
				return Token{Kind: TokenIdent, Value: val, Offset: start, Line: lx.line, Col: startCol}
			}
			// Operators
			ops := "+-*/%==!=<><=>=&&||!"
			if strings.ContainsRune(ops, rune(ch)) {
				...
				return Token{Kind: TokenOp, Value: lx.src[start:lx.offset], Offset: start, Line: lx.line, Col: startCol}
			}
			...
			lx.offset++
			lx.col++
			return Token{Kind: TokenError, Value: lx.src[start:lx.offset], Offset: start, Line: lx.line, Col: startCol}
		}
	}
