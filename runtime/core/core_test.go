...
func TestCompareNumericMix(t *testing.T) {
	if Compare(1, 2.0) != -1 {
		t.Errorf("int vs float compare failed")
	}
	if Compare(2.5, 2) != 1 {
		t.Errorf("float vs int compare failed")
	}
	if Compare(3, 3.0) != 0 {
		t.Errorf("int/float equality compare failed")
	}
}

func TestEqual(t *testing.T) {
	if !Equal(2, 2.0) {
		t.Errorf("Equal should treat 2 and 2.0 as equal")
	}
	if Equal("a", "b") {
		t.Errorf("Equal strings failed")
	}
	if !Equal("x", "x") {
		t.Errorf("Equal equal strings failed")
	}
}
